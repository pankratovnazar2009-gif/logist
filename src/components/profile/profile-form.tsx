...
import { LANGUAGE_OPTIONS, type AppUser } from "@/lib/types";
...
export function ProfileForm({ user }: { user: AppUser }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (saving) return;
    if (fullName.trim() && fullName.trim().length < 2) return setSave({ status: "error", message: p.errors.nameShort, field: "name" });
    if (nip && !/^\d{10}$/.test(nip)) return setSave({ status: "error", message: p.errors.nipDigits, field: "nip" });
    ...
    try {
      ...
      if (err instanceof ApiError && err.code === "nip_not_found") return setSave({ status: "error", message: p.errors.nipNotFound, field: "nip" });
      if (err instanceof ApiError && err.code === "invalid_input") return setSave({ status: "error", message: p.errors.invalid, field: "email" });
      setSave({ status: "error", message: p.errors.generic });
    }
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-6">
      <section className="card flex flex-col gap-4" aria-labelledby="personal-heading">
        ...
        <Field id="fullName" label={p.fullName} error={fieldError("name")}>
          <input id="fullName" className="input" autoComplete="name" value={fullName} onChange={(e) => setFullName(e.target.value)} maxLength={80} />
        </Field>
        <Field id="email" label={p.email} error={fieldError("email")} hint={p.emailHint}>
        ...
        <Field id="phone" label={p.phone} hint={p.phoneHint}>
      ...
      <section className="card flex flex-col gap-4" aria-labelledby="company-heading">
        ...
        <Field id="nip" label={p.nip} error={fieldError("nip")} hint={p.nipHint}>
          <input id="nip" className="input mono" inputMode="numeric" maxLength={10} value={nip} onChange={(e) => setNip(e.target.value.replace(/\D/g, ""))} />
        </Field>
        <Field id="company" label={p.companyName} hint={companyVerified ? p.companyFromGus : undefined}>
      ...
      <section className="card flex flex-col gap-5" aria-labelledby="prefs-heading">
        <h2 id="prefs-heading" className="font-[family-name:var(--font-display)] text-lg font-bold">
          {isCarrier ? p.prefsFleet : p.prefsLanguages}
        </h2>
        <ChipGroup label={p.languagesLabel} options={LANGUAGE_OPTIONS} selected={languages} onChange={setLanguages} />
        {isCarrier && <ChipGroup label={p.bodyType} options={truckOptions(m)} selected={truckTypes} onChange={setTruckTypes} />}
        {isCarrier && <ChipGroup label={p.directions} options={regionOptions(m)} selected={routes} onChange={setRoutes} />}
      </section>
      ...
      <div className="flex flex-wrap items-center gap-4">
        ...
        <p role="status" className="text-sm" style={{ color: save.status === "error" ? "var(--color-danger)" : "var(--color-success)" }}>
          {save.status === "saved" ? m.common.saved : save.status === "error" && !save.field ? save.message : ""}
        </p>
      </div>
    </form>
  );
