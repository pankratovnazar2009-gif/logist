...
/**
 * Находит перевозчиков, которым подходит груз (направление + тип кузова), и шлёт SMS со ссылкой
 * на карточку груза. Дедупликация через notifications_log (load_id, user_id) — повторный вызов не спамит.
 */
export async function matchAndNotifyForLoad(loadId: string): Promise<{ notified: number }> {
  ...
  const { data: alreadyNotified } = await supabase.from("notifications_log").select("user_id").eq("load_id", loadId);
  ...
  const link = `${appBaseUrl()}/loads/${loadId}`;
