...
- Тексты — в  (польский — исходник, остальные типизированы по нему: пропущенный ключ не даст собрать проект).
- Цвета — токены в ; тёмная тема переопределяет те же имена под .
